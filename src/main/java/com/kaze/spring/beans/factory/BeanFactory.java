...
public interface BeanFactory {
    ...
    /**
     用于解引用（获取）FactoryBean实例本身，并将其与FactoryBean创建的Bean区分开来。
     例如，如果名为myJndiObject的Bean是一个FactoryBean，那么通过获取&myJndiObject将返回该工厂实例，而非工厂通过getObject()方法创建的实例。
     */
    ...
    /**
     * 返回指定 Bean 的一个实例（可能是共享（单例）的或独立（原型）的）。
     * 该方法使得 Spring BeanFactory 能够替代传统的单例（Singleton）或原型（Prototype）设计模式。对于单例 Bean，调用者可以保留对返回对象的引用。
     * 提示：这个方法永远不会返回null,即使用工厂方法显示的返回了null也会返回NullBean代替null
     */
