...
import com.kaze.spring.core.SerializableTypeWrapper.FieldTypeProvider;
import com.kaze.spring.core.SerializableTypeWrapper.MethodParameterTypeProvider;
import com.kaze.spring.core.SerializableTypeWrapper.TypeProvider;
...
/**
 * 用于解决泛型擦除。
 * 封装一个Java类型，提供对其超类型、接口实现以及泛型参数的访问能力，并支持最终解析为具体类类型。
 * ResolvableType 可以通过以下方式获取：字段、方法参数、方法返回类型、类。
 * 该类的大多数方法会返回 ResolvableType 对象，支持链式调用。
 *
 * 例子：
...
public class ResolvableType implements Serializable {
	...
	/**
	 * 当没有可用值时返回的 ResolvableType。应该采用 NONE 而非空值(null)，以便多个方法调用可以安全地进行链式操作。
	 */
	...
	/**
	 * 私有构造方法，用于为缓存键创建新的ResolvableType实例，且不进行预先解析。
	 */
	...
	/**
	 * 私有构造方法，用于为缓存值创建新的ResolvableType实例，包含预先解析和预计算的哈希值。
	 */
	...
	/**
	 * 私有构造函数用于创建新的 ResolvableType 实例，适用于未缓存的情况，具有预先解析，但没有哈希值计算
	 */
	...
	/**
	 * 私有构造函数用于基于 Class 创建新的 ResolvableType 实例。
	 * 创建的 ResolvableType 实例仅表示原始 Class 类型（如 String），不涉及泛型参数（如 List<String>）或数组类型（如 String[]）
	 */
	...
	public Class<?> getRawClass() {
		...
		//ParameterizedType表示带有泛型参数的类型，getRawType可以返回原始类型（未泛型化的类/接口），例如 List<String> 中的 List
